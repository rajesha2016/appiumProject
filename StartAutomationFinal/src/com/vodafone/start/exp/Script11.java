...
public class Script11 {
	...
	private static Logger log = Logger.getLogger(Script11.class);
	...
	public void validateCTASMS(String deviceName) 	{
			...
			try{
				...
				for(String s : contexts){
					...
					if(!s.equalsIgnoreCase(currentContext)){
						...
						log.info("switch to native SMS app");
						driver.context(s);
						break;
					}
				}
				
				log.info("verify user is navigated to SMS");
				mainPage.verifySMSTextboxExists();
				
				log.info("take screenshot");
				util.takeScreenShot(this.getClass().getSimpleName() + "-NavigatedToSMS");
